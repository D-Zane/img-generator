...
func Generate(urlPath []string) (*bytes.Buffer, error) {
	var (
		err      error
	...
	for i, val := range urlPath {
		...
		case 2:
			if val != "" {
				...
				if err != nil {
					//log.Println("Con not parse 'imgH', err: ", err)
					return nil, err
				}
			}
		case 3:
	...
	rgb, err := hx.ToRGB()
	if err != nil {
		//log.Println("Con not parse 'imgColor', err: ", err)
		return nil, err
	}
